...
public class UndirectedUnweightedGraph<TYPE> {

    private final ArrayList<TYPE> vertices; //lista que armazena os dados inserido no grafo
    private boolean[][] adjacencyMatrix; //matriz que forma a estrutura do grafo
    private int currentCapacity; //capacidade atual do vetor adjacencyMatrix

    public UndirectedUnweightedGraph(int startingCapacity) {
    ...
    private boolean[][] startAdjacencyMatrix(int size) {
        this.currentCapacity = size;
        boolean[][] newAdjacencyMatrix = new boolean[size][size];
        ...
        for (int lineIndex = 0; lineIndex < this.currentCapacity; lineIndex++) {
            for (int columnIndex = 0; columnIndex < this.currentCapacity; columnIndex++) {
                newAdjacencyMatrix[lineIndex][columnIndex] = false; //inicia como arestas inexistentes
            }
        }
    ...
    private void increaseCapacity() {
        ...
        boolean[][] newAdjacencyMatrix = this.startAdjacencyMatrix(this.currentCapacity * 2);
    ...
    public void addEdge(TYPE startVertex, TYPE endVertex) {
        ...
        this.adjacencyMatrix[startIndex][endIndex] = true;
        this.adjacencyMatrix[endIndex][startIndex] = true;
    }
    ...
    public void removeEdge(TYPE startVertex, TYPE endVertex) {
        ...
        this.adjacencyMatrix[startIndex][endIndex] = false;
        this.adjacencyMatrix[endIndex][startIndex] = false;
    }
    ...
    private void depthSearch(int currentVertex, boolean[] visited) {
        ...
        for (int adjacentVertexIndex = 0; adjacentVertexIndex < visited.length; adjacentVertexIndex++) {
            ...
            if (!visited[adjacentVertexIndex] && this.adjacencyMatrix[currentVertex][adjacentVertexIndex]) {
    ...
    private void breadthSearch(Queue<Integer> verticesToProcess, boolean[] verticesVisited) {
        ...
        for (int adjacentVertexIndex = 0; adjacentVertexIndex < this.size(); adjacentVertexIndex++) {
            //passa pelo vertice adjacente ainda nao visitado
            if (!verticesVisited[adjacentVertexIndex] && this.adjacencyMatrix[currentVertex][adjacentVertexIndex]) {
                System.out.println("Veio de " + this.vertices.get(currentVertex) + " para " + this.vertices.get(adjacentVertexIndex));
    ...
    public void dijkstra(TYPE src) {
        ...
        for (int count = 0; count < this.size() - 1; count++) {
            ...
            for (int adjacentVertexIndex = 0; adjacentVertexIndex < this.size(); adjacentVertexIndex++) {
                 ...
                 */
                int distanceIncrement = this.adjacencyMatrix[currentElementIndex][adjacentVertexIndex] ? 1 : 0;
                int distance = shorterDistance[currentElementIndex] + distanceIncrement;
